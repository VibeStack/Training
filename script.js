    ...
    for (let key in requiredDayData) {
      if (key === "id") {
        ...
        for (const element in requiredDayData[key]) {
          const noOfElements = Object.values(
            requiredDayData[key][element]
          ).length;
          for (let i = 0; i < noOfElements; i++) {
            if (element === "ol") {
              const noOfLiInOlTags = Object.values(
                requiredDayData[key][element]
              ).length;
            ...
            if (element === "a") {
              for (const aTagKeys in requiredDayData[key][element]) {
                createTags(
                  `${element}`,
                  { href: `${requiredDayData[key][element][aTagKeys]}`, target:"_blank"},
                  `${aTagKeys}`,
                  sectionTags
                );
              }
